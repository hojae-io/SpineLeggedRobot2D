function [] = buildLegDynamics_sym()
    % Define each element of q, q_dot, q_ddot, u, F, and par individually
    syms q_0 q_1 q_2 q_3 q_4 q_5 q_6 real
    syms q_dot_0 q_dot_1 q_dot_2 q_dot_3 q_dot_4 q_dot_5 q_dot_6 real
    syms q_ddot_0 q_ddot_1 q_ddot_2 q_ddot_3 q_ddot_4 q_ddot_5 q_ddot_6 real
    syms u_0 u_1 u_2 u_3 real
    syms F_0 F_1 F_2 F_3 real
    syms p_0 p_1 p_2 p_3 p_4 p_5 p_6 p_7 p_8 p_9 p_10 p_11 p_12 p_13 p_14 p_15 p_16 p_17 p_18 p_19 p_20 p_21 real

    % Create symbolic vectors from individual elements for convenience
    q = [q_0; q_1; q_2; q_3; q_4; q_5; q_6];
    q_dot = [q_dot_0; q_dot_1; q_dot_2; q_dot_3; q_dot_4; q_dot_5; q_dot_6];
    q_ddot = [q_ddot_0; q_ddot_1; q_ddot_2; q_ddot_3; q_ddot_4; q_ddot_5; q_ddot_6];
    u = [u_0; u_1; u_2; u_3];
    F = [F_0; F_1; F_2; F_3];
    par = [p_0; p_1; p_2; p_3; p_4; p_5; p_6; p_7; p_8; p_9; p_10; p_11; p_12; p_13; p_14; p_15; p_16; p_17; p_18; p_19; p_20; p_21];
    ...
    %% Body kinematics
    e_hat = cos(q(3))*i_hat + sin(q(3))*j_hat;
    r_O = [q(1); q(2); 0];
    r_O_dot = ddt(r_O);
    w_O = q_dot(3);

    r_Of = r_O + par_lengths(1) * e_hat;
    r_Oh = r_O - par_lengths(1) * e_hat;

    %% Fore-leg kinematics
    e1f_hat = sin(q(3) + q(4))*i_hat - cos(q(3) + q(4))*j_hat;
    e2f_hat = sin(q(3) + q(4) + q(5))*i_hat - cos(q(3) + q(4) + q(5))*j_hat;
    ...
    w_1f = q_dot(3) + q_dot(4);
    w_2f = q_dot(3) + q_dot(4) + q_dot(5);
    w_3f = q_dot(3) + q_dot(4) + q_dot(5);
    w_4f = q_dot(3) + q_dot(4);
    ...
    V_0 = par_mass(1) * par_gravity * dot(r_O, j_hat);
    ...
    T = simplify(T_0 + T_1f + T_2f + T_3f + T_4f + T_1f_rotor + T_2f_rotor + ...
                       T_1h + T_2h + T_3h + T_4h + T_1h_rotor + T_2h_rotor);
    V = simplify(V_0 + V_g1f + V_g2f + V_g3f + V_g4f + V_g1h + V_g2h + V_g3h + V_g4h);

    Q_tau1f = M2Q(u(1) * k_hat, w_1f * k_hat);
    Q_tau1f_reaction = M2Q(-u(1) * k_hat, w_O * k_hat);
    ...
    Q = simplify(Q_tau1f + Q_tau1f_reaction + Q_tau2f + Q_tau2f_reaction + ...
                 Q_tau1h + Q_tau1h_reaction + Q_tau2h + Q_tau2h_reaction);
    ...
    matlabFunction(A, 'File', 'codegen/rigid/A_fn', 'Vars', {z, par});
    
    % Save b as a MATLAB function
    matlabFunction(b, 'File', 'codegen/rigid/b_fn', 'Vars', {z, u, par});

    % Save G as a MATLAB function
    matlabFunction(G, 'File', 'codegen/rigid/G_fn', 'Vars', {z, par});
    
    % Save C as a MATLAB function
    matlabFunction(C, 'File', 'codegen/rigid/C_fn', 'Vars', {z, par});
    
    % Save energy E as a MATLAB function
    matlabFunction(E, 'File', 'codegen/rigid/energy_fn', 'Vars', {z, par});
    
    % Save InvDynamics as a MATLAB function
    matlabFunction(InvDynamics, 'File', 'codegen/rigid/InvDynamics_fn', 'Vars', {q, q_dot, q_ddot, par});
    
    % Save the position of the front end-effector
    matlabFunction(r_Ef(1:2), 'File', 'codegen/rigid/pos_front_ee', 'Vars', {z, par});
    
    % Save the velocity of the front end-effector
    matlabFunction(r_Ef_dot(1:2), 'File', 'codegen/rigid/vel_front_ee', 'Vars', {z, par});
    
    % Save the Jacobian of the front end-effector
    matlabFunction(Jf, 'File', 'codegen/rigid/J_front_ee', 'Vars', {z, par});
    
    % Save the derivative of the Jacobian of the front end-effector
    matlabFunction(dJf, 'File', 'codegen/rigid/dJ_front_ee', 'Vars', {z, par});

    % Save the position of the hind end-effector
    matlabFunction(r_Eh(1:2), 'File', 'codegen/rigid/pos_hind_ee', 'Vars', {z, par});
    
    % Save the velocity of the hind end-effector
    matlabFunction(r_Eh_dot(1:2), 'File', 'codegen/rigid/vel_hind_ee', 'Vars', {z, par});
    
    % Save the Jacobian of the hind end-effector
    matlabFunction(Jh, 'File', 'codegen/rigid/J_hind_ee', 'Vars', {z, par});

    % Save the derivative of the Jacobian of the hind end-effector
    matlabFunction(dJh, 'File', 'codegen/rigid/dJ_hind_ee', 'Vars', {z, par});
    
    % Save the position of the front hip
    matlabFunction(r_Of(1:2), 'File', 'codegen/rigid/pos_front_hip', 'Vars', {z, par});
    
    % Save the position of the hind hip
    matlabFunction(r_Oh(1:2), 'File', 'codegen/rigid/pos_hind_hip', 'Vars', {z, par});
    
    % Save keypoints as a MATLAB function
    matlabFunction(keypoints, 'File', 'codegen/rigid/keypoints_fn', 'Vars', {z, par});
    
    % Save the Center of Mass (CoM)
    matlabFunction(CoM, 'File', 'codegen/rigid/CoM_fn', 'Vars', {z, par});
    
    % Save the derivative of the Center of Mass (CoM)
    matlabFunction(CoM_dot, 'File', 'codegen/rigid/CoM_dot_fn', 'Vars', {z, par});
